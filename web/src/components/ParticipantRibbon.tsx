    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 5, position: 'relative' }}>
      ...
      {adding ? (
        <input
          ...
          onBlur={addOne}
          onKeyDown={(e) => { if (e.key === 'Enter') void addOne(); if (e.key === 'Escape') { setName(''); setAdding(false); } }}
          placeholder="Name"
          style={{
            padding: '5px 10px', borderRadius: 14, border: '1.5px dashed var(--border)',
            background: 'transparent', fontSize: 12, width: 90,
          }}
        />
      ) : (
        <button
      ...
      {editing && (
        <ParticipantEditPopover
          ...
          onChanged={() => { onChanged(); setEditing(null); }}
        />
      )}
    </div>
  );
    ...
    >
      <div
        ...
        style={{
          ...
          display: 'grid', gap: 10, fontSize: 12,
        }}
      >
        <input
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <button onClick={() => void save()} style={primaryBtn}>Save</button>
          <button
            onClick={() => void remove()}
            style={{
              ...secondaryBtn,
          ...
          >{confirmDel ? 'Click again' : 'Remove'}</button>
          <div style={{ flex: 1 }} />
...
function chipStyle(hue: number | null): React.CSSProperties {
  ...
  return {
    ...
    background: `oklch(72% 0.13 ${h})`, color: '#fff',
...
const secondaryBtn: React.CSSProperties = {
  padding: '6px 12px', borderRadius: 6, border: '1.5px solid var(--border)',
