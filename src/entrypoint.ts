...
import { backfillItemDerivations, backfillTripSlugs } from './boot-backfill.js';
...
async function main(): Promise<void> {
  ...
  const slugs = backfillTripSlugs(db);
  if (slugs.updated > 0) {
    console.log(`Assigned slugs to ${slugs.updated} pre-existing trip(s).`);
  }
